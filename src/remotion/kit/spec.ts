 ...
 */
/**
 * ── 길이 정본 (2026-08-11 재조사로 개정) ────────────────────────────────────
 * 최적 길이는 «하나»가 아니다 — 플랫폼 알고리즘이 보는 지표가 다르다.
 *   YouTube Shorts 50~58s  시청 «시간»
 *   TikTok 바이럴  24~38s  완주«율»      ← 대표가 기억한 34초가 여기
 *   Reels 교육형   30~45s  루프 반복
 * RPM 은 길이와 무관하다(15초든 3분이든 동일) — 길이는 «조회수»로만 수익에 닿는다.
 * 그래서 대본은 «길게» 쓰고 kit/variants.ts 가 플랫폼별로 잘라낸다.
 * 구판(42s 고정, 36~50)은 유튜브 시청시간을 버리고 있었다.
 */
export const LENGTH = { targetSec: 54, minSec: 28, maxSec: 58 } as const;
