...
// ============================================================================
export async function processPortfolioBatch(tickers: string[], mode: 'full' | 'price' = 'full') {
    const startTime = Date.now();

    if (!tickers || tickers.length === 0) return { results: [], meta: { count: 0, elapsed: 0, source: 'empty' } };

    // ═══ [CACHE WARMER] Cache-first fast path (full mode only) ═══
    // price mode is already fast (snapshot only), skip cache for it
    if (mode === 'full') {
        try {
            ...
            if (cachedCount === tickers.length) {
                // Full cache hit — fetch only live prices from Polygon snapshot
                ...
                const results = tickers.map(ticker => {
                    ...
                    return {
                        ...
                        realtime: {
                            ...
                            // Portfolio-specific: tripleA not available in cache, will be null
                            threeDay: analysis.return3d,
            ...
            console.warn('[Portfolio Batch] Cache check failed, using fallback:', e);
        }
    }
