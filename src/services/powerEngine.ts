...
export function computeQualityTier(
    ...
    try {
        ...
        alphaV3 = calculateAlphaScore({
            ...
            ndxChangePct,
            vixValue,
            vixChangePct: evidence?.macro?.vix?.changePct ?? null,
            tltChangePct: evidence?.macro?.tlt?.changePct ?? null,
            gldChangePct: evidence?.macro?.gld?.changePct ?? null,
            fearGreedScore: evidence?.macro?.fearGreed?.score ?? null,
            dxy: evidence?.macro?.dxy?.value ?? null,
            realYieldStance: evidence?.macro?.realYield?.stance ?? null,
