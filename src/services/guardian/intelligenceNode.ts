...
const REALITY_PROMPTS: Record<Locale, (ctx: IntelligenceContext) => string> = {
    ko: (ctx) => {
        ...
        // Macro context
        const yieldContext = ctx.us10y !== undefined ? `- 10Y 금리: ${ctx.us10y?.toFixed(2)}% / 스프레드: ${ctx.spread2s10s?.toFixed(2) || '?'}%` : '';

        return `
        당신은 월가 최고의 기술적 분석가입니다. 수치를 해석하여 실전 매매 인사이트를 제공합니다.

        **📊 현재 시장 데이터:**
        - RLSI 점수: ${ctx.rlsiScore.toFixed(0)}점 (${rlsiLevel})
        - 나스닥: ${ctx.nasdaqChange >= 0 ? '+' : ''}${ctx.nasdaqChange.toFixed(2)}% (${priceAction})
        - VIX: ${ctx.vix.toFixed(1)} (${vixLevel})
        - 거래량(RVOL): ${ctx.rvol.toFixed(2)}x (${rvolLevel})
        ${yieldContext}

        **🎯 분석 프레임워크:**
        1. RLSI 65+ & 가격 상승 → 건강한 상승, 추세 추종 유효
        2. RLSI 65+ & 가격 하락 → 스마트머니 매집, 저가 매수 기회
        3. RLSI 45 이하 & 가격 상승 → 가짜 랠리, 추격 매수 금지
        4. RLSI 45 이하 & 가격 하락 → 약세 확인, 리스크 오프
        5. VIX 25+ → 공포 극대화, 역발상 매수 검토
        6. RVOL 1.5+  → 기관 참여 활발, 움직임 신뢰도 높음

        **✍️ 출력 (정확히 이 형식으로, 2문장 이내):**
        현재 시장 상태에 대한 핵심 인사이트를 자연스러운 한국어로 작성하세요.
        - "[진단]" "[결론]" 같은 레이블 사용 금지
        - 전문가가 트레이더에게 설명하듯이 작성
        - 구체적인 매매 관점 포함 (저가 매수, 관망, 리스크 관리 등)
        - 공백 포함 80자 이내

        **예시 (참고용, 그대로 복사 금지):**
        - "기관 자금이 여전히 유입 중이며 약세는 일시적 조정으로 판단됩니다. 눌림목 매수 유효."
        - "내부 지표가 취약한 가운데 상승은 공매도 커버 성격이 강합니다. 신규 매수 보류 권장."
        - "VIX 급등 속에서도 RLSI가 버티고 있어 패닉은 과도합니다. 역발상 매수 타이밍 탐색."
    `;
    },
    en: (ctx) => `
