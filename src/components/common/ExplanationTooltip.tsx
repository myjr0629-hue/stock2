...
// Displays explanations for indicators on hover/click
    ...
    return (
        <div className={`relative inline-flex items-center gap-1 ${className}`}>
            ...
            <button
                ...
                aria-label={`View ${explanation.label} explanation`}
            >
            ...
            {isOpen && (
                <div className="absolute z-50 bottom-full left-0 mb-2 w-72 bg-[#1A1F26] border border-slate-600 rounded-lg shadow-xl p-4 text-left">
                    ...
                    <div className="space-y-2 text-[11px] text-slate-300">
                        <div>
                            <span className="text-slate-500 font-medium">{t('definition')}: </span>
                            {explanation.meaning}
                        </div>
                        <div>
                            <span className="text-slate-500 font-medium">{t('interpretation')}: </span>
                            {explanation.interpretation}
                        </div>
                        <div>
                            <span className="text-slate-500 font-medium">{t('action')}: </span>
                            {explanation.action}
                        </div>
                        {explanation.caution && (
                            <div className="text-amber-400/80">
                                <span className="font-medium">⚠️ {t('caution')}: </span>
                                {explanation.caution}
                            </div>
                        )}
                    </div>
