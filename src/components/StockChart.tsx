                            ...
                            {isIntraday && nbbo && nbbo.bid > 0 && nbbo.ask > 0 && (() => {
                                ...
                                return (
                                    <div className="absolute top-2 left-1/2 -translate-x-1/2 z-30 flex items-center gap-1.5 px-3 py-1 rounded-md bg-slate-900/70 backdrop-blur-sm border border-white/10">
                                        ...
                                        <span className="text-[12px] font-bold text-slate-300 font-jakarta"><CardTooltip tooltip={{ ko: 'NBBO — 미국 전 거래소 통합 최우선 매수/매도 호가. Bid(최우선 매수가)와 Ask(최우선 매도가) 간 Spread가 유동성 지표로 사용됩니다. Spread < 0.05%: 유동성 매우 풍부 (대형주 수준) | 0.05~0.15%: 정상 범위 | > 0.15%: 유동성 부족, 체결 슬리피지 주의.', en: 'NBBO — Consolidated best bid/offer across all U.S. exchanges. The bid-ask spread serves as a real-time liquidity gauge. Spread < 0.05%: Deep liquidity (mega-cap level) | 0.05–0.15%: Normal | > 0.15%: Thin liquidity, watch for slippage.', ja: 'NBBO — 全米取引所統合ベスト・ビッド/オファー。ビッド・アスク・スプレッドはリアルタイムの流動性指標です。スプレッド < 0.05%: 高流動性（大型株水準）| 0.05〜0.15%: 標準 | > 0.15%: 流動性低下、スリッページに注意。' }}>NBBO</CardTooltip></span>
                                        <span className="text-slate-600">|</span>
                                        {/* Bid */}
