    ...
    if (mode === 'blur') {
        return (
            <div className={`relative rounded-xl flex flex-col overflow-hidden ${className}`}
                ...
                <div className="pointer-events-none select-none flex-1 flex flex-col overflow-hidden" style={{ filter: `blur(${blurPx ? blurPx + 'px' : (requiredTier === 'pro' ? (compact ? '4px' : '10px') : (compact ? '2.5px' : '4.5px'))})`, minHeight: minHeight || undefined }}>
                    {children}
                </div>
