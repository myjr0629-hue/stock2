    ...
    return (
        <section className="relative z-10">
            <div className="rounded-xl border border-slate-700/40 overflow-hidden"
                ...
                {/* PC TreeMap */}
                <div className="hidden md:block" style={{ height: 450 }}>
                    <ReactECharts
                        option={option}
                ...
                <div className="block md:hidden bg-[#0d1117] max-h-[550px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent rounded-b-xl border-t border-slate-800">
                    {treeData.map((sector, idx) => {
                        ...
                        return (
                        <div key={sector.name || idx} className="border-b border-slate-800/50 last:border-0">
                            {/* Mobile Sector Header */}
                            <div className="px-4 py-2.5 bg-gradient-to-r from-slate-900/80 to-transparent flex justify-between items-center sticky top-0 z-10 backdrop-blur-md border-b border-white/5">
                                <span className="text-[13px] font-bold text-slate-200 tracking-wider font-jakarta">{sector.name}</span>
                                <span className={`text-[13px] font-mono font-bold ${sector.avgChange >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
