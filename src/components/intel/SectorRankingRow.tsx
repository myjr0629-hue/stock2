...
const i18n: Record<Locale, {
    ...
    ko: {
        ...
        live: 'LIVE',
        closed: 'CLOSED',
        flowIn: (t, amt) => `${t} 종목에 ${amt} 순유입 중. 기관/스마트머니의 집중 유입 관측.`,
        flowOut: (t, amt) => `${t} 종목에서 ${amt} 순유출 중. 대규모 매도 또는 헷지 포지셔닝 관측.`,
        flowBalanced: '전 종목 자금 흐름 균형 상태. 뚜렷한 방향성 부재.',
        squeezeImm: (t, dir, dist) => `${t} 종목이 ${dir}까지 ${dist}% 거리에 임박. 급격한 가격 변동 가능성 관측.`,
        squeezeNear: (t, dir, dist) => `${t} 종목이 ${dir}까지 ${dist}%로 접근 중. 레벨 접근 모니터링 중.`,
        squeezeSafe: '전 종목 핵심 레벨과 충분한 거리 유지. 급변동 리스크 낮음.',
        painHigh: (t, pct, dir) => `${t} 종목이 Max Pain 대비 ${pct}% 괴리. 만기일 ${dir} 수렴 압력 관측.`,
    ...
    en: {
        ...
        live: 'LIVE',
        closed: 'CLOSED',
        flowIn: (t, amt) => `${t} seeing ${amt} net inflow. Institutional/smart money accumulation observed.`,
        flowOut: (t, amt) => `${t} seeing ${amt} net outflow. Large-scale selling or hedge positioning observed.`,
        flowBalanced: 'Balanced capital flow across all tickers. No clear directional bias.',
        squeezeImm: (t, dir, dist) => `${t} within ${dist}% of ${dir}. Sharp price movement potential observed.`,
        squeezeNear: (t, dir, dist) => `${t} approaching ${dir} at ${dist}%. Level proximity under monitoring.`,
        squeezeSafe: 'All tickers maintaining safe distance from key levels. Low volatility risk.',
        painHigh: (t, pct, dir) => `${t} diverged ${pct}% from Max Pain. Expiry convergence pressure toward ${dir} observed.`,
    ...
    ja: {
        ...
        live: 'LIVE',
        closed: 'CLOSED',
        flowIn: (t, amt) => `${t}に${amt}の純流入中。機関投資家の集中流入を観測。`,
        flowOut: (t, amt) => `${t}から${amt}の純流出中。大規模売却またはヘッジポジショニングを観測。`,
        flowBalanced: '全銘柄で資金フロー均衡状態。明確な方向性なし。',
        squeezeImm: (t, dir, dist) => `${t}が${dir}まで${dist}%の距離に迫っています。急激な価格変動の可能性を観測。`,
        squeezeNear: (t, dir, dist) => `${t}が${dir}まで${dist}%に接近中。レベル接近モニタリング中。`,
        squeezeSafe: '全銘柄が主要レベルから十分な距離を維持。急変動リスク低。',
        painHigh: (t, pct, dir) => `${t}がMax Pain比${pct}%乖離。満期日の${dir}収束圧力を観測。`,
