        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 200 180" preserveAspectRatio="none">
            <defs>
                <linearGradient id="csGrad" x1="0" y1="1" x2="0" y2="0">
                    <stop offset="0%" stopColor="#22d3ee" stopOpacity="0.007" />
            ...
            {/* Semi-circle gauge */}
            <path d="M30 140 A70 70 0 0 1 170 140" stroke="#22d3ee" strokeWidth="2.5" fill="none" opacity="0.07" />
            <path d="M45 140 A55 55 0 0 1 155 140" stroke="#22d3ee" strokeWidth="1.5" fill="none" opacity="0.04" />
            {/* Gauge needle */}
            <line x1="100" y1="140" x2="55" y2="85" stroke="#22d3ee" strokeWidth="2" strokeLinecap="round" opacity="0.08" />
            <circle cx="100" cy="140" r="4" fill="#22d3ee" opacity="0.07" />
            {/* Scale marks */}
            <line x1="30" y1="140" x2="35" y2="130" stroke="#22d3ee" strokeWidth="1.5" opacity="0.055" />
            <line x1="50" y1="95" x2="58" y2="100" stroke="#22d3ee" strokeWidth="1.5" opacity="0.055" />
            <line x1="100" y1="70" x2="100" y2="78" stroke="#22d3ee" strokeWidth="1.5" opacity="0.055" />
            <line x1="150" y1="95" x2="142" y2="100" stroke="#22d3ee" strokeWidth="1.5" opacity="0.055" />
            <line x1="170" y1="140" x2="165" y2="130" stroke="#22d3ee" strokeWidth="1.5" opacity="0.055" />
            {/* % watermark */}
            <text x="152" y="48" fontSize="34" fill="#22d3ee" opacity="0.035" fontWeight="bold">%</text>
        </svg>
    );
}
            ...
            <div className="relative z-10">
                ...
                <div className="grid grid-cols-5 gap-1.5 text-center mb-2">
                    <div className="bg-emerald-500/10 rounded p-1.5 border border-emerald-500/20">
                        <div className="text-base font-black text-white">{aggregated.strongBuy}</div>
                        <div className="text-[11px] text-emerald-400 uppercase">Strong Buy</div>
                    </div>
                    <div className="bg-emerald-500/5 rounded p-1.5 border border-emerald-500/10">
                        <div className="text-base font-black text-white">{aggregated.buy}</div>
                        <div className="text-[11px] text-emerald-400/80 uppercase">Buy</div>
                    </div>
                    <div className="bg-slate-700/30 rounded p-1.5 border border-slate-600/30">
                        <div className="text-base font-black text-white">{aggregated.hold}</div>
                        <div className="text-[11px] text-white/60 uppercase">Hold</div>
                    </div>
                    <div className="bg-rose-500/5 rounded p-1.5 border border-rose-500/10">
                        <div className="text-base font-black text-white">{aggregated.sell}</div>
                        <div className="text-[11px] text-rose-400/80 uppercase">Sell</div>
                    </div>
                    <div className="bg-rose-500/10 rounded p-1.5 border border-rose-500/20">
                        <div className="text-base font-black text-white">{aggregated.strongSell}</div>
                        <div className="text-[11px] text-rose-400 uppercase">Strong Sell</div>
                    </div>
                </div>
                        ...
                        {Object.entries(recommendations).map(([symbol, rec]) => {
                            ...
                            return (
                                <div key={symbol} className="flex items-center gap-1.5">
                                    <img src={getLogoUrl(symbol)} alt={symbol} className="w-4 h-4 rounded-full flex-shrink-0" />
                                    <span className="text-[11px] font-bold text-white w-10">{symbol}</span>
                                    ...
                                    <span className={`text-[11px] font-bold ${sentiment === 'bullish' ? 'text-emerald-400' : sentiment === 'bearish' ? 'text-rose-400' : 'text-white/50'}`}>
                                        {bullPct.toFixed(0)}%
                                    </span>
                                </div>
                            );
