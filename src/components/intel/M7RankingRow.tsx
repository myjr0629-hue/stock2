...
const i18n: Record<Locale, {
    ...
    ko: {
        ...
        live: 'LIVE',
        closed: 'CLOSED',
        flowIn: (t, amt) => `${t}에 ${amt} 규모 콜 프리미엄 집중 유입. 기관 참여자의 강세 포지셔닝 관측.`,
        flowOut: (t, amt) => `${t}에서 ${amt} 규모 풋 프리미엄 유출. 하방 헷지 또는 약세 포지셔닝 관측.`,
        flowBalanced: '전 종목 자금 흐름 균형 상태. 뚜렷한 방향성 부재.',
        squeezeImm: (t, dir, dist) => `${t} ${dir} ${dist}% 근접. 돌파 시 감마 스퀴즈로 급변동 발생 가능성 관측.`,
        squeezeNear: (t, dir, dist) => `${t} ${dir}까지 ${dist}% — 레벨 접근 중. 돌파 여부 모니터링.`,
        squeezeSafe: '전 종목 핵심 레벨과 충분한 거리 유지. 급변동 리스크 낮음.',
        painHigh: (t, pct, dir) => `${t} Max Pain 대비 ${pct}% 괴리. 만기 수렴 압력으로 ${dir} 발생 가능성 관측.`,
    ...
    en: {
        ...
        live: 'LIVE',
        closed: 'CLOSED',
        flowIn: (t, amt) => `${t} receiving ${amt} concentrated call premium inflow. Institutional bullish positioning observed.`,
        flowOut: (t, amt) => `${t} showing ${amt} put premium outflow. Downside hedging or bearish positioning observed.`,
        flowBalanced: 'Balanced capital flow across all tickers. No clear directional bias.',
        squeezeImm: (t, dir, dist) => `${t} at ${dist}% from ${dir}. Gamma squeeze potential observed on proximity.`,
        ...
        painHigh: (t, pct, dir) => `${t} at ${pct}% divergence from Max Pain. Expiry convergence pressure toward ${dir} observed.`,
    ...
    ja: {
        ...
        live: 'LIVE',
        closed: 'CLOSED',
        flowIn: (t, amt) => `${t}に${amt}規模のコールプレミアム集中流入。機関投資家の強気ポジショニングを観測。`,
        flowOut: (t, amt) => `${t}から${amt}規模のプットプレミアム流出。ヘッジまたは弱気ポジショニングを観測。`,
