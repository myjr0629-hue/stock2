// Bio Pulse Calendar Data API - Earnings & Recommendations
...
export async function GET(req: NextRequest) {
    try {
        ...
        const [earningsResults, recommendationResults] = await Promise.all([
            Promise.all(BIO_PULSE_TICKERS.map(async (symbol) => {
                try {
                    const earnings = await getEarningsCalendar(symbol, fromDate, toDate);
                    return earnings;
                } catch (e) {
                    console.error(`[BioPulse API] Earnings error for ${symbol}:`, e);
                    return [];
                }
            })),
            Promise.all(BIO_PULSE_TICKERS.map(async (symbol) => {
                try {
                    ...
                    console.error(`[BioPulse API] Recommendation error for ${symbol}:`, e);
        ...
        return NextResponse.json({
            ...
            recommendations,
            tickers: BIO_PULSE_TICKERS,
            timestamp: new Date().toISOString()
        });
    } catch (error) {
        console.error('[BioPulse Calendar API] Error:', error);
        return NextResponse.json({
            earnings: [], recommendations: {}, tickers: BIO_PULSE_TICKERS,
