      ...
      case 'education': {
        ...
        for (const lang of langs) {
          ...
          if (thChEdu) {
            ...
            // Cache EN OG CDN URL for Pinterest (education_pin runs 5min later)
            if (lang === 'en' && eduImage && !dryRun) {
              try { await setInCache(`marketing:edu_og_cdn:${dateKey}`, eduImage, 86400); } catch {}
            }
...
for (const lang of langs) {
      ...
      case 'spacex_spotlight': {
        ...
        // === SpaceX content from Redis (pre-generated by daily-content?type=spacex) ===
        const spacexContent = await loadContent('spacex', dateKey);
        if (!spacexContent) {
          return noContent('spacex', dateKey);
        }
        ...
        if (PLATFORM_ALLOW[action]?.has('telegram') && (region === 'en' || region === 'all')) {
          const tgNewsBody = (spacexContent.en as any)?.text || 'Institutional positioning data updated.';
          const tgSpaceXText = [
            `🚀 SpaceX IPO — Daily Update`,
            '',
            tgNewsBody,
            '',
            '*Observation only — not financial advice.',
