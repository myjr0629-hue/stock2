...
export default function MobileWatchlistPage({ locale, initialWatchlist, initialFullData }: {
    ...
    // ── batch API가 정확한 changePct/extendedChangePct를 직접 반환하므로 별도 보정 불필요 ──
    // [FIX 2026-05-06] 이전 correctedItems 역산 로직 제거:
    //   batch API buildBasePrice()에서 dailyCloses 기반 정확한 본장%, PRE% 계산 완료
    //   역산 로직이 오히려 정확한 값을 오염시킴 (e.g. NVDA 1.84% → 3.05%)
