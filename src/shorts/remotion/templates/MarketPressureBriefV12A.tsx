// ============================================================================
// MarketPressureBrief V12A — SPY Shock Hook
// ============================================================================
...
const HookTextV12A: React.FC = () => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: exit, zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v12a_01.mp3')} volume={0.8} />
      ...
      <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 10, transform: `scale(${interpolate(slam, [0, 1], [1.4, 1])})` }}>
        <div style={{ color: '#ffffff', fontSize: SCALE.hero, fontWeight: 900, fontFamily: TYPE.family, letterSpacing: '-0.02em', textShadow: SHADOW.hero, lineHeight: 1.1, opacity: slam }}>
          SPY IS <span style={{ color: BRAND.amber }}>1.3%</span>
        </div>
        <div style={{ color: '#ffffff', fontSize: SCALE.hero, fontWeight: 900, fontFamily: TYPE.family, letterSpacing: '-0.02em', textShadow: SHADOW.hero, lineHeight: 1.1, opacity: slam }}>
          FROM A WALL
        </div>
...
// 5.5 - 8.5s
const WhyCareTextV12A: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: Math.min(pop, exit), zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v12a_02.mp3')} volume={0.8} />
...
// 8.5 - 12.5s
const MeaningTextV12A: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: Math.min(pop, exit), zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v12a_03.mp3')} volume={0.8} />
...
// 12.5 - 17.0s
const ProductDifferenceV12A: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: exit, zIndex: Z.wallViz + 10, background: BRAND.bg }}>
      <Audio src={staticFile('shorts/audio/v12a_04.mp3')} volume={0.8} />
...
// 17.0 - 21.0s
const BrandCTAV12A: React.FC<{ cta: string }> = ({ cta }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: r, zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v12a_05.mp3')} volume={0.8} />
      <Audio src={staticFile('shorts/audio/v11_sfx_pulse.mp3')} volume={0.2} />
...
const ProceduralDataLayerV12A: React.FC<{ data: ShortsVideoInput['structureVisual'] }> = ({ data }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Phase logic
  const isPhase1 = frame >= S(2.5) && frame < S(5.5); // Data Reveal
  const isPhase2 = frame >= S(5.5) && frame < S(8.5); // Pressure Build
  const isPhase3 = frame >= S(8.5) && frame < S(12.5); // Map
  ...
  if (isPhase2) {
    const approach = interpolate(frame, [S(5.5), S(8.5)], [0, 0.8], { extrapolateRight: 'clamp' });
    priceY = toY(price) + (toY(callWall) - toY(price)) * approach * 0.35;
  }

  const bracketGrow = isPhase1 
    ? spring({ frame: Math.max(0, frame - S(3.0)), fps, config: { damping: 12, stiffness: 240 } })
    : 1;
  const bracketH = Math.max(0, priceY - wallY);

  const mapFrame = isPhase3 ? Math.max(0, frame - S(9.0)) : 0;
  ...
  return (
    <div style={cameraStyle}>
      ...
      {isPhase2 && (
        <div style={{ position: 'absolute', left: 100, right: 350, top: wallY + 4, height: Math.max(0, priceY - wallY - 4), background: `linear-gradient(180deg, ${BRAND.coral}40 0%, ${BRAND.cyan}15 50%, transparent 100%)`, borderLeft: `2px dashed ${BRAND.coral}40`, opacity: interpolate(frame, [S(5.5), S(6.0)], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }) }} />
      )}
...
export const MarketPressureBriefV12A: React.FC<MarketPressureBriefProps> = (props) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BRAND.bg }}>
      ...
      {structureVisual && <ProceduralDataLayerV12A data={structureVisual} />}

      <Sequence from={0} durationInFrames={S(2.5)}><HookTextV12A /></Sequence>
      {/* 2.5 - 5.5 is data layer reveal */}
      <Sequence from={S(5.5)} durationInFrames={S(3.0)}><WhyCareTextV12A /></Sequence>
      <Sequence from={S(8.5)} durationInFrames={S(4.0)}><MeaningTextV12A /></Sequence>
      <Sequence from={S(12.5)} durationInFrames={S(4.5)}><ProductDifferenceV12A /></Sequence>
      <Sequence from={S(17.0)} durationInFrames={S(4.0)}><BrandCTAV12A cta={cta} /></Sequence>
