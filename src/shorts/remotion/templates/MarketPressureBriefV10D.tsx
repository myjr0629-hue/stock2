// ============================================================================
// MarketPressureBrief V10D — Audio Integration
// ============================================================================
...
// ---------------------------------------------------------------------------
// BEAT COMPONENTS (with Audio)
// ---------------------------------------------------------------------------

const HookTextV10D: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: exit, zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v10c/beat1.mp3')} />
...
const ConcretePayoffV10D: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: Math.min(drop, exit), zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v10c/beat2.mp3')} />
...
const WhyCareTextV10D: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: Math.min(pop, exit), zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v10c/beat3.mp3')} />
      <div style={{ position: 'absolute', top: 300, left: 0, right: 0, textAlign: 'center' }}>
...
const ProductDifferenceV10D: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: exit, zIndex: Z.wallViz + 10, background: BRAND.bg }}>
      <Audio src={staticFile('shorts/audio/v10c/beat4.mp3')} />
...
const MeaningTextV10D: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: Math.min(pop, exit), zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v10c/beat5.mp3')} />
...
const TensionTextV10D: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: Math.min(r, exit), zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v10c/beat6.mp3')} />
      <div style={{ position: 'absolute', top: 300, left: 0, right: 0, textAlign: 'center' }}>
...
const BrandCTAV10D: React.FC<{ cta: string }> = ({ cta }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: r, zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v10c/beat7.mp3')} />
...
// ---------------------------------------------------------------------------
const ProceduralDataLayerV10D: React.FC<{ data: ShortsVideoInput['structureVisual'] }> = ({ data }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  if (isPhase3) {
    ...
    scale = interpolate(frame, [S(16.5), S(19.5)], [1.5, 1.8]); 
    translateY = interpolate(frame, [S(16.5), S(19.5)], [50, 100]); 
  }
...
export const MarketPressureBriefV10D: React.FC<MarketPressureBriefProps> = (props) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BRAND.bg }}>
      ...
      {structureVisual && <ProceduralDataLayerV10D data={structureVisual} />}

      <Sequence from={0} durationInFrames={S(2.5)}><HookTextV10D /></Sequence>
      <Sequence from={S(2.5)} durationInFrames={S(3.5)}><ConcretePayoffV10D /></Sequence>
      <Sequence from={S(6.0)} durationInFrames={S(3.0)}><WhyCareTextV10D /></Sequence>
      <Sequence from={S(9.0)} durationInFrames={S(4.0)}><ProductDifferenceV10D /></Sequence>
      <Sequence from={S(13.0)} durationInFrames={S(3.5)}><MeaningTextV10D /></Sequence>
      <Sequence from={S(16.5)} durationInFrames={S(3.0)}><TensionTextV10D /></Sequence>
      <Sequence from={S(19.5)} durationInFrames={S(2.5)}><BrandCTAV10D cta={cta} /></Sequence>
