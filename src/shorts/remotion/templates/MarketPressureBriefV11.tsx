// ============================================================================
// MarketPressureBrief V11 — Final Audio Mix & Sync
// ============================================================================
...
// ---------------------------------------------------------------------------
// BEATS WITH AUDIO AND SFX
// ---------------------------------------------------------------------------

// 0.0 - 2.4s
const HookTextV11: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const slam = spring({ frame, fps, config: { damping: 12, stiffness: 400, mass: 0.4 } });
  const exit = interpolate(frame, [S(2.2), S(2.4)], [1, 0], { extrapolateRight: 'clamp' });

  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: exit, zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v11_voice_01.mp3')} volume={0.8} />
      <Audio src={staticFile('shorts/audio/v11_sfx_impact.mp3')} volume={0.3} />
      
      {frame === S(0.4) && <Audio src={staticFile('shorts/audio/v11_sfx_scan.mp3')} volume={0.2} />}
...
// 2.4 - 6.0s
const ConcretePayoffV11: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const drop = spring({ frame, fps, config: { damping: 14, stiffness: 250, mass: 0.5 } });
  const exit = interpolate(frame, [S(3.4), S(3.6)], [1, 0], { extrapolateRight: 'clamp' }); 

  return (
    <div style={{ position: 'absolute', inset: 0, opacity: Math.min(drop, exit), zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v11_voice_02.mp3')} volume={0.8} />
...
const WhyCareTextV11: React.FC = () => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: Math.min(pop, exit), zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v11_voice_03.mp3')} volume={0.8} />
      <div style={{ position: 'absolute', top: 300, left: 0, right: 0, textAlign: 'center' }}>
...
// 9.0 - 12.2s (Meaning)
const MeaningTextV11: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const pop = spring({ frame, fps, config: { damping: 14, stiffness: 220 } });
  const exit = interpolate(frame, [S(2.9), S(3.2)], [1, 0], { extrapolateRight: 'clamp' });

  return (
    <div style={{ position: 'absolute', inset: 0, opacity: Math.min(pop, exit), zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v11_voice_04.mp3')} volume={0.8} />
...
// 12.2 - 16.2s (Product Toggle)
const ProductDifferenceV11: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const split = spring({ frame, fps, config: { damping: 12, stiffness: 280, mass: 0.6 } });
  const exit = interpolate(frame, [S(3.8), S(4.0)], [1, 0], { extrapolateRight: 'clamp' });

  return (
    <div style={{ position: 'absolute', inset: 0, opacity: exit, zIndex: Z.wallViz + 10, background: BRAND.bg }}>
      <Audio src={staticFile('shorts/audio/v11_voice_05.mp3')} volume={0.8} />
...
// 16.2 - 21.0s (CTA)
const BrandCTAV11: React.FC<{ cta: string }> = ({ cta }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: r, zIndex: Z.hookText }}>
      <Audio src={staticFile('shorts/audio/v11_voice_06.mp3')} volume={0.8} />
      <Audio src={staticFile('shorts/audio/v11_sfx_pulse.mp3')} volume={0.2} />
...
// ---------------------------------------------------------------------------
// DYNAMIC PROCEDURAL DATA VIZ
// ---------------------------------------------------------------------------
const ProceduralDataLayerV11: React.FC<{ data: ShortsVideoInput['structureVisual'] }> = ({ data }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Adjusted phase timings
  const isPhase1 = frame >= S(2.4) && frame < S(6.0);
  const isPhase2 = frame >= S(6.0) && frame < S(9.0);
  const isPhase3 = frame >= S(9.0) && frame < S(12.2); // Map
  ...
  const bracketGrow = isPhase1 
    ? spring({ frame: Math.max(0, frame - S(3.4)), fps, config: { damping: 12, stiffness: 240 } })
    : 1;
  const bracketH = Math.max(0, priceY - wallY);

  const mapFrame = isPhase3 ? Math.max(0, frame - S(9.5)) : 0;
...
const AtmosphereV11: React.FC = () => {
  const frame = useCurrentFrame();
  const hookImg = interpolate(frame, [S(2.2), S(2.4)], [0.6, 0], { extrapolateRight: 'clamp' });
  const isPressureVisible = (frame >= S(2.4) && frame < S(9.0)) || (frame >= S(9.0) && frame < S(12.2));
...
export const MarketPressureBriefV11: React.FC<MarketPressureBriefProps> = (props) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BRAND.bg }}>
      ...
      {structureVisual && <ProceduralDataLayerV11 data={structureVisual} />}

      <Sequence from={0} durationInFrames={S(2.4)}><HookTextV11 /></Sequence>
      <Sequence from={S(2.4)} durationInFrames={S(3.6)}><ConcretePayoffV11 /></Sequence>
      <Sequence from={S(6.0)} durationInFrames={S(3.0)}><WhyCareTextV11 /></Sequence>
      
      {/* Script reversed these beats! Map is first, then product toggle */}
      <Sequence from={S(9.0)} durationInFrames={S(3.2)}><MeaningTextV11 /></Sequence>
      <Sequence from={S(12.2)} durationInFrames={S(4.0)}><ProductDifferenceV11 /></Sequence>
      
      {/* 16.2 to 21.0s */}
      <Sequence from={S(16.2)} durationInFrames={S(4.8)}><BrandCTAV11 cta={cta} /></Sequence>
