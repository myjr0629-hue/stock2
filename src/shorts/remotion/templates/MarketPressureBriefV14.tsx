// ============================================================================
// MarketPressureBrief V14 — First Real Upload Candidate
// ============================================================================
...
// 0.0 - 2.0s
const HardDataHookV14: React.FC = () => {
  const frame = useCurrentFrame();
    ...
    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: exit, zIndex: Z.hookText }}>
      ...
      <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 10, transform: `scale(${interpolate(slam, [0, 1], [1.4, 1])})` }}>
        <div style={{ color: '#ffffff', fontSize: SCALE.hero, fontWeight: 900, fontFamily: TYPE.family, letterSpacing: '-0.02em', textShadow: SHADOW.hero, lineHeight: 1.1, opacity: slam }}>
          SPY <span style={{ color: BRAND.amber }}>1.3%</span> BELOW
        </div>
        <div style={{ color: BRAND.coral, fontSize: SCALE.hero - 10, fontWeight: 900, fontFamily: TYPE.family, letterSpacing: '-0.02em', textShadow: SHADOW.coral, lineHeight: 1.1, opacity: slam }}>
          HIDDEN CALL WALL
        </div>
      </div>
    </div>
...
// 2.0 - 4.0s
const MissingLayerV14: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
...
// 9.8 - 14.0s
const ProductDifferenceV14: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
...
// 14.0 - 17.2s
const ProductPromiseV14: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const pop = spring({ frame, fps, config: { damping: 14, stiffness: 220 } });
...
// 17.2 - 20.5s
const BrandCTAV14: React.FC<{ cta: string }> = ({ cta }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
...
const ProceduralDataLayerV14: React.FC<{ data: ShortsVideoInput['structureVisual'] }> = ({ data }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Phase logic
  // 0.0-2.0s: Hook (1.3% Hero, no price)
  // 2.0-4.0s: FOMO (Normal line fades)
  // 4.0-6.8s: Meaning (Bracket locks, pressure glow, price appears small)
  // 6.8-9.8s: Map assembly
  const isPhase1 = frame < S(2.0);
  const isPhase2 = frame >= S(2.0) && frame < S(4.0);
  const isPhase3 = frame >= S(4.0) && frame < S(6.8);
  const isPhase4 = frame >= S(6.8) && frame < S(9.8);
  const isPhase5 = frame >= S(14.0) && frame < S(17.2); // Product promise, background map slightly visible
  ...
  const wallBrightness = isPhase3 ? interpolate(frame, [S(4.0), S(5.5)], [0.4, 0.9], { extrapolateRight: 'clamp' }) : 0.4;
  
  // In phase 1 & 2 we show a static bracket to represent "1.3%". It locks / tightens in phase 3.
  ...
  return (
    <div style={cameraStyle}>
      ...
      {/* Pressure Glow */}
      {isPhase3 && (
        <div style={{ position: 'absolute', left: 100, right: 350, top: wallY + 4, height: Math.max(0, priceY - wallY - 4), background: `linear-gradient(180deg, ${BRAND.coral}40 0%, ${BRAND.cyan}15 50%, transparent 100%)`, borderLeft: `2px dashed ${BRAND.coral}40`, opacity: interpolate(frame, [S(4.0), S(5.5)], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }) }} />
      )}
      ...
      {/* Price Line (fades out in Phase 2) */}
      {(isPhase1 || isPhase2 || isPhase3) && (
        <>
          <div style={{ position: 'absolute', left: 60, top: priceY - 2, height: 8, width: 600, background: BRAND.cyan, boxShadow: `0 0 20px ${BRAND.cyan}`, borderRadius: 4, opacity: interpolate(frame, [S(2.0), S(3.0)], [1, 0.2], { extrapolateRight: 'clamp' }) }} />
          <div style={{ position: 'absolute', left: 644, top: priceY - 16, width: 32, height: 32, borderRadius: '50%', background: BRAND.text, border: `6px solid ${BRAND.cyan}`, boxShadow: `0 0 20px ${BRAND.cyan}`, opacity: interpolate(frame, [S(2.0), S(3.0)], [1, 0.2], { extrapolateRight: 'clamp' }) }} />
        </>
      )}

      {/* SPY Price Label (Only small and secondary in Phase 3) */}
      {showPrice && (
        <div style={{ position: 'absolute', left: 700, top: priceY - 10, opacity: interpolate(frame, [S(4.0), S(4.5)], [0, 0.4], { extrapolateRight: 'clamp' }) }}>
...
export const MarketPressureBriefV14: React.FC<MarketPressureBriefProps> = (props) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BRAND.bg }}>
      ...
      {structureVisual && <ProceduralDataLayerV14 data={structureVisual} />}

      <Sequence from={0} durationInFrames={S(2.0)}><HardDataHookV14 /></Sequence>
      <Sequence from={S(2.0)} durationInFrames={S(2.0)}><MissingLayerV14 /></Sequence>
      <Sequence from={S(4.0)} durationInFrames={S(2.8)}><WhyCareTextV14 /></Sequence>
      <Sequence from={S(6.8)} durationInFrames={S(3.0)}><PressureMapV14 /></Sequence>
      <Sequence from={S(9.8)} durationInFrames={S(4.2)}><ProductDifferenceV14 /></Sequence>
      <Sequence from={S(14.0)} durationInFrames={S(3.2)}><ProductPromiseV14 /></Sequence>
      <Sequence from={S(17.2)} durationInFrames={S(3.3)}><BrandCTAV14 cta={cta} /></Sequence>
