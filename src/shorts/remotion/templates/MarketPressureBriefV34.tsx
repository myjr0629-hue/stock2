// ============================================================================
// MarketPressureBrief V34 — Alert Boot & 24s Rebuild
// ============================================================================
// Complete 24-second (720 frames at 30fps) event-based insight short.
// Integrates:
// 1. Signature Scan Alert Boot Intro (0.0s - 0.4s, f0 - f12)
...
// 5. High-Impact Insight Payoff: "Not a call. A pressure map." (12.5s - 18.0s, f375 - f540)
...
export type MarketPressureBriefV34Props = ShortsVideoInput;
  ...
  return (
    <AbsoluteFill style={{ background: '#020409', overflow: 'hidden', zIndex: Z.bg }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', bottom: 8, right: 12, color: 'rgba(255,255,255,0.3)', fontSize: 12, fontFamily: MONO_FAMILY }}>[MODE: EVENT_SHOCK_V34]</div>
      </div>
    </AbsoluteFill>
  );
};
...
// ─── Directional Institutional Flow Particles V34 ───────────
const FlowParticlesV34: React.FC<{
...
const SafeCaptionOverlay: React.FC<{ captions: CaptionSegment[]; frame: number }> = ({ captions, frame }) => {
  if (frame >= S(18.0)) return null;
  if (!captions || captions.length === 0) return null;

  const activeCaption = captions.find(c => frame >= c.startFrame && frame < c.endFrame);
  if (!activeCaption || activeCaption.text === "NEAR SPY'S $600 WALL" || activeCaption.text === "STRUCTURE SCAN ACTIVE") return null;

  // Scene 01 targets Y=1410 (bottom of screen), others target Y=380 (top of screen)
  ...
  const isPressureMap = activeCaption.text === 'NOT A CALL. A PRESSURE MAP.' || activeCaption.text === 'WHERE PRESSURE CLUSTERS';
...
const AudioEngine: React.FC = () => (
  <>
    <Audio src={staticFile('shorts/audio/v34_voice.mp3')} volume={0.98} />
...
// ═════════════════════════════════════════════════════════════════════════════
const Scene00_AlertBoot: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{ background: '#020409', zIndex: Z.wallViz + 20 }}>
      ...
      <div style={{
        ...
        background: BRAND.cyan, boxShadow: `0 0 30px ${BRAND.cyanGlow}, 0 0 10px #ffffff`,
      }} />
      ...
      <div style={{
        ...
        <svg width="220" height="220" viewBox="246 247 530 530" fill="none" style={{
          ...
          filter: glitch ? 'hue-rotate(90deg) blur(1px)' : 'none',
        }}>
        ...
        <div style={{
          fontFamily: MONO_FAMILY, fontSize: 32, fontWeight: 900, color: BRAND.cyan,
          letterSpacing: '0.2em', textShadow: `0 0 15px ${BRAND.cyanGlow}`, marginTop: 45,
        ...
        <div style={{
          ...
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: BRAND.coral, display: 'inline-block', animation: 'pulse 1s infinite' }} />
          SYSTEM_BOOT_SCAN_ACTIVE...
        </div>
      </div>
    </AbsoluteFill>
  );
  ...
  return (
    <AbsoluteFill style={{ zIndex: Z.wallViz }}>
      ...
      <div style={{
        ...
        <FlowParticlesV34 startY={wallY - 520} endY={priceY - 520} count={16} speedFactor={1.6} />
      </div>
...
const ContinuousTransformChart: React.FC<{ frame: number; scene: 'scene2' | 'scene3' | 'scene4' }> = ({ frame, scene }) => {
  const chartTop = 520;
  ...
  // Product Unlock Reveal Pulse (triggered exactly at Scene 2's end: f105 - f225, sweep at f210 - f222)
  ...
  const revealProgress = scene === 'scene2'
    ...
    : 1.0; // fully revealed in subsequent scenes
            ...
            return (
              <>
                <svg width="1080" height="1920" style={{ position: 'absolute', top: 0, left: 0, zIndex: Z.data }}>
                  <path d={`M 80,${chartTop + 450} Q 400,${chartTop + 280} 600,${activePriceY} T 1000,${chartTop + 320}`} fill="none" stroke={revealProgress > 0.1 ? BRAND.cyan : `rgba(255,255,255,${pathColor})`} strokeWidth="4" />
                </svg>

                {isScannerSweepFrame && <ScannerLine progress={sweepProgress} />}
                ...
                <div style={{ zIndex: Z.data, opacity: revealProgress }}>
                  ...
                  {/* Put Floor */}
                  <div style={{
                    position: 'absolute', left: 80, right: 80, top: putFloorY, height: 6, background: BRAND.cyan,
                    boxShadow: `0 0 45px ${BRAND.cyanGlow}`,
                  }} />
                  <div style={{ position: 'absolute', left: 100, top: putFloorY + 12, color: BRAND.cyan, fontFamily: MONO_FAMILY, fontSize: 18, fontWeight: 900, textShadow: SHADOW.cyan }}>
                    [PUT FLOOR $580.00]
                  </div>
          ...
          {scene === 'scene3' && (() => {
            ...
            return (
              <>
                ...
                <FlowParticlesV34 startY={callWallY} endY={activePriceY} count={24} speedFactor={2.2} />
              </>
            );
          })()}
          ...
          {scene === 'scene4' && (() => {
            return (
              <>
                ...
                <div style={{ position: 'absolute', left: 100, top: putFloorY + 12, color: BRAND.cyan, fontFamily: MONO_FAMILY, fontSize: 18, fontWeight: 900, textShadow: SHADOW.cyan, zIndex: Z.data }}>
                  [PUT FLOOR $580.00]
                </div>
                ...
                <div style={{
                  position: 'absolute', left: 600 - 18, top: activePriceY - 18, width: 36, height: 36,
                  borderRadius: '50%', background: BRAND.cyan,
                  boxShadow: `0 0 40px ${BRAND.cyanGlow}, 0 0 10px #ffffff`, zIndex: Z.data + 2,
                }} />
                ...
                <FlowParticlesV34 startY={callWallY} endY={gammaFlipY} count={24} frameOffset={120} />
              </>
            );
          })()}
        </div>
...
// ─── SCENE 05: CTA WITH LOOP HOOK (18.0s - 24.0s, f540 - f720) ─────────────────────────────
const Scene05_CTA: React.FC = () => {
  const frame = useCurrentFrame();
  const { durationInFrames } = useVideoConfig();
  const currentFrameAbsolute = frame + S(18.0);
    ...
    <AbsoluteFill style={{ zIndex: Z.hookText }}>
      ...
      <div style={{
        ...
        <div style={{ marginTop: 22 }}>
          ...
          }}>[SEE THE HIDDEN MARKET STRUCTURE]</span>
        </div>
      </div>
      ...
      {/* Subtle loop scan and Wall line blend inoutro */}
      {isLoopWindow && (
        <div style={{
...
// ─── MAIN COMPOSITION ENTRYPOINT (V34) ───────────────────────────────────────
export const MarketPressureBriefV34: React.FC<MarketPressureBriefV34Props> = ({ captions, disclaimer }) => {
  const frame = useCurrentFrame();
  const { durationInFrames } = useVideoConfig();
  ...
  return (
    <AbsoluteFill style={{ background: '#020409', overflow: 'hidden' }}>
      ...
      <LivingTerminalBackground sceneName="v34-terminal-depth" />

      {/* 2. Scene Sequences (V34 REBUILT 24s TIMELINE) */}
      {/* Scene 00: Signature Scan Alert Boot (0.0s - 0.4s, 12f) */}
      <Sequence from={0} durationInFrames={S(0.4)}>
        <Scene00_AlertBoot />
      </Sequence>

      {/* Scene 01: Event Shock (0.4s to 3.5s, 93f, f12 - f105) */}
      <Sequence from={S(0.4)} durationInFrames={S(3.1)}>
        <Scene01_EventShock />
      </Sequence>

      {/* Scene 02: Normal Chart vs Hidden (3.5s to 7.5s, 120f, f105 - f225) */}
      <Sequence from={S(3.5)} durationInFrames={S(4.0)}>
        <ContinuousTransformChart frame={frame - S(3.5)} scene="scene2" />
      </Sequence>

      {/* Scene 03: Gap Tension Dynamic Zoom & Screen Shake (7.5s to 12.5s, 150f, f225 - f375) */}
      <Sequence from={S(7.5)} durationInFrames={S(5.0)}>
        <ContinuousTransformChart frame={frame - S(7.5)} scene="scene3" />
      </Sequence>

      {/* Scene 04: Insight Payoff (12.5s to 18.0s, 165f, f375 - f540) */}
      <Sequence from={S(12.5)} durationInFrames={S(5.5)}>
        <ContinuousTransformChart frame={frame - S(12.5)} scene="scene4" />
      </Sequence>

      {/* Scene 05: Outro Brand CTA + Silver Loop (18.0s to 24.0s, 180f, f540 - f720) */}
      <Sequence from={S(18.0)} durationInFrames={S(6.0)}>
        <Scene05_CTA />
      </Sequence>
      ...
      {/* 4. Legal Compliance Footer (Scenes 01 to 05) */}
      <Sequence from={S(0.4)} durationInFrames={S(23.6)}>
...
export default MarketPressureBriefV34;
