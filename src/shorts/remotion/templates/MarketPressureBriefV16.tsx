// ============================================================================
// MarketPressureBrief V16 — Upload Candidate (Data-First Recovery)
// Recovers V14.1 concrete data clarity. Rejects V15 abstract emptiness.
// Pure Remotion/SVG/CSS. No Replicate wallpaper.
// ============================================================================
...
const DataVizLayer: React.FC<{ data: ShortsVideoInput['structureVisual'] }> = ({ data }) => {
  ...
  return (
    <div style={{ position: 'absolute', top: 300, left: 0, width: 1080, height: 900, zIndex: Z.wallViz, transform: `translateY(${translateY}px)` }}>
      ...
      {/* SPY Price label (Phase 3 only, faint, far from 1.3%) */}
      {isPressure && (
        <div style={{ position: 'absolute', left: 700, top: priceY - 10, opacity: interpolate(frame, [S(4.2), S(4.8)], [0, 0.3], { extrapolateRight: 'clamp' }) }}>
          ...
          <div style={{ color: BRAND.cyan, fontSize: SCALE.mapLabel - 10, fontWeight: 800, fontFamily: TYPE.family, marginTop: -2 }}>${price.toFixed(2)}</div>
        </div>
      )}
...
const HardDataHook: React.FC = () => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: Z.hookText }}>
      ...
      <div style={{ position: 'absolute', top: 750, left: 0, right: 0, textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 5, alignItems: 'center', transform: `scale(${interpolate(slam, [0, 1], [1.3, 1])})` }}>
...
const ProductContrast: React.FC = () => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: Z.wallViz + 10, background: BRAND.bg }}>
      ...
      {/* BOTTOM HALF: SignumHQ Layer (vivid, structured, alive) */}
...
const ProductPromise: React.FC = () => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, opacity: pop, zIndex: Z.hookText }}>
      ...
      <div style={{ position: 'absolute', top: 300, left: 0, right: 0, textAlign: 'center' }}>
        <div style={{ color: BRAND.text, fontSize: SCALE.insight, fontWeight: 800, fontFamily: TYPE.family, textShadow: SHADOW.hero, transform: `translateY(${(1 - pop) * 20}px)` }}>
          SEE THE STRUCTURE
        </div>
        <div style={{ color: BRAND.cyan, fontSize: SCALE.insight, fontWeight: 900, fontFamily: TYPE.family, textShadow: SHADOW.cyan, marginTop: 10, transform: `translateY(${(1 - pop) * 30}px)` }}>
          BEHIND PRICE
        </div>
      </div>
    </div>
...
// Main Composition
// ─────────────────────────────────────────────────────────────────────────────
export const MarketPressureBriefV16: React.FC<MarketPressureBriefProps> = (props) => {
