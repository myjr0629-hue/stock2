 ...
 * Output path modified for current session
 */
...
## Legacy Engines Quintile Analysis
| Quintile | N | Score Range | Avg Return (3D) | Med Return (3D) | Win% | Sharpe |
|---|---|---|---|---|---|---|
...
### V7.0.0 Evaluation
* **Monotonicity**: ${v7Results.monotonic ? 'Validated. Higher score bands consistently lead to higher returns.' : 'Non-monotonic. Score does not linearly align with returns.'}
* **Pearson r**: **${v7Results.correlation ? v7Results.correlation.toFixed(4) : 'N/A'}** (${v7Results.correlation ? (Math.abs(v7Results.correlation) < 0.05 ? 'No correlation' : Math.abs(v7Results.correlation) < 0.15 ? 'Weak correlation' : 'Moderate correlation') : 'N/A'}).
* **Edge**: BUY edge is **${v7Results.buyStats ? (v7Results.buyStats.edge >= 0 ? '+' : '') + v7Results.buyStats.edge.toFixed(2) + 'pp' : 'N/A'}** over the market. AVOID edge is **${v7Results.avoidStats ? (v7Results.avoidStats.edge >= 0 ? '+' : '') + v7Results.avoidStats.edge.toFixed(2) + 'pp' : 'N/A'}**.

### Comparison
${v7Results.monotonic && !legacyResults.monotonic ? '* V7.0.0 successfully restored linear monotonicity that was broken in Legacy versions.' : ''}
${v7Results.correlation > legacyResults.correlation ? `* Pearson r increased from ${legacyResults.correlation.toFixed(4)} to ${v7Results.correlation.toFixed(4)}, showcasing better predictability.` : ''}
`;

  const reportPath = 'C:\\Users\\seamo\\.gemini\\antigravity\\brain\\59556e1d-65b3-42f3-9761-66893d1fae0a\\backtest_v7_results.md';
  fs.writeFileSync(reportPath, md);
  console.log(`\nReport written to: ${reportPath}`);
  
  // Also print key numbers to console
  console.log('\n=== KEY RESULTS ===');
  console.log(`V7 Pairs: ${v7Results.count}, Legacy Pairs: ${legacyResults.count}`);
  console.log(`V7 Correlation: ${v7Results.correlation ? v7Results.correlation.toFixed(4) : 'N/A'}`);
  console.log(`V7 Monotonic: ${v7Results.monotonic}`);
  console.log(`V7 BUY(>=65) - Win: ${v7Results.buyStats ? v7Results.buyStats.win.toFixed(1) : 'N/A'}%, Avg: ${v7Results.buyStats ? v7Results.buyStats.avg.toFixed(2) : 'N/A'}%`);
  console.log(`V7 AVOID(<40) - Win: ${v7Results.avoidStats ? v7Results.avoidStats.win.toFixed(1) : 'N/A'}%, Avg: ${v7Results.avoidStats ? v7Results.avoidStats.avg.toFixed(2) : 'N/A'}%`);
  if (v7Results.qStats) {
    console.log('\nV7 Quintiles:');
    v7Results.qStats.forEach(q => console.log(`  ${q.name}: avg=${q.avg.toFixed(2)}%, win=${q.win.toFixed(1)}%, sharpe=${q.sharpe.toFixed(3)}`));
  }
}
