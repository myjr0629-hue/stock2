...
// 출력:  public/ad/tall-whale.png (+ 좌표는 콘솔)
// ============================================================================
...
(async () => {
  ...
  const hit = await page.evaluate(() => {
    const el = [...document.querySelectorAll('button,[role="tab"],div,span')]
      .find((e) => /^WHALE\s*&\s*DP$/i.test((e.innerText || '').trim())
        && e.getBoundingClientRect().height > 18 && e.getBoundingClientRect().height < 80);
    if (!el) return false;
    el.click(); return true;
  });
  console.log(hit ? '  WHALE & DP 탭 클릭' : '  ✗ 탭을 못 찾음');
  await sleep(4000);
  ...
  await dismiss(page);
  await sleep(1800);
  // 탭이 초기화됐을 수 있으니 WHALE & DP 를 다시 누른다
  await page.evaluate(() => {
    const el = [...document.querySelectorAll('button,[role="tab"],div,span')]
      .find((e) => /^WHALE\s*&\s*DP$/i.test((e.innerText || '').trim())
        && e.getBoundingClientRect().height > 18 && e.getBoundingClientRect().height < 80);
  ...
  const raw = path.join(TMP, 'tall-whale.png');
  await page.screenshot({ path: raw, type: 'png' });
  downscale(raw, path.join(OUT, 'tall-whale.png'));
  console.log(`  ✔ public/ad/tall-whale.png  (스크롤 ${h}px)`);
  ...
  const boxes = await page.evaluate((labs, kk) => {
    ...
    return out;
  }, ['WHALE', 'DARK POOL', 'BLOCK', 'SWEEP', 'LARGE'], k);
  ...
  const words = await page.evaluate(() =>
    (document.body.innerText || '').split('\n').map((s) => s.trim())
      .filter((s) => /whale|dark pool|block|sweep/i.test(s)).slice(0, 14));
  console.log('\n  화면에 실제로 있는 문구:');
