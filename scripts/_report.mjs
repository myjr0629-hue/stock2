...
if (out.length >= 2) {
  ...
  console.log('\n  ⛔ 주의 (2026-08-23 정정): 「테스트1이 테스트2 때문에 잘렸다」는 근거가 없다.');
  console.log('     테스트1 감속은 15:58 에 시작됐고 테스트2 는 16:05 에야 조회를 받았다.');
  console.log('     같은 감속 모양이 테스트1 자신의 1차 파동에도 있었다 — 파동 소멸은 이 포맷의 고유 모양이다.');
  console.log('     다만 테스트1이 3차 파동을 잃었는지는 여전히 알 수 없다.');
}
