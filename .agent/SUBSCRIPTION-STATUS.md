...
## ✅ 2026-09-08 확인 — 스토어 쪽은 «전부 승인»됐다

위 표(1~5)는 2026-09-02 밤 작업으로 이미 끝났는데 표만 갱신이 안 돼 있었다.
오늘 ASC API 로 실제 상태를 조회한 결과:

| 항목 | 상태 |
|---|---|
| 앱 버전 1.5 · 1.6 | **READY_FOR_SALE** |
| 구독 `com.signumhq.app.pro.monthly` | **APPROVED** ✅ |
| 구독 그룹 `SIGNUM Pro` (22205273) | 로컬라이제이션 3종 완비 |
| 가격 | 175개국 (예: AE 39.99 / AF 9.99 / US 9.99) |

### RevenueCat 실호출 검증 (2026-09-08)
공개 SDK 키로 REST 를 직접 호출해 **양 플랫폼 모두** 확인:
```
GET /v1/subscribers/{id}/offerings   (X-Platform: ios | android)
→ current_offering_id: default
→ 패키지 $rc_monthly → com.signumhq.app.pro.monthly
```
코드가 `offerings.current` 에서 `packageType==='MONTHLY'` 로 찾는 경로와 일치한다.

### 페이월 화면 검증 (2026-09-08)
프리뷰 배포에서 ko/en/ja 3개 언어 실측 — **잘림 0 · 가로넘침 0**,
페이월 내부 터치타깃 전부 44×44 이상(닫기 44×44 확인).
문구도 요건 충족: 상품명 · 가격(스토어 현지값) · 기간 · 포함내용 ·
자동갱신/해지 안내 · 구매 복원 · 이용약관 · 개인정보처리방침 · 닫기.

> `/[locale]/paywall-preview` 가 이제 **프리뷰 배포에서도 열린다**(운영 도메인은 그대로 막힘).
> dev 서버를 띄우지 않고 프리뷰 URL 로 바로 확인·스크린샷을 찍을 수 있다.

## ✅ 2026-09-08 — 라이브 (IAP_LIVE = true, 운영 반영)

### 시뮬레이터에서 «네이티브로» 실측한 것

| 확인 | 결과 |
|---|---|
| 설정 맨 위 «SIGNUM Pro · 광고 없이 · 월 구독» | 뜬다 (웹에서는 안 뜬다) |
| 페이월 열림 | **버그였다 → 고침** (아래) |
| 가격 | **스토어에서 실제로 내려온다** ($9.99 · 미국 스토어프론트) |
| 구매 누름 | StoreKit 결제 시트 도달 (계정 로그인 요구 = 정상) |
| `setPro(true)` | 배너 사라지고 **그 자리를 콘텐츠가 채운다** (죽은 띠 없음) |
| 웹(브라우저) | 유료 진입점이 **하나도 안 뜬다** |

### ★★ 찾은 버그 — 설정에서 페이월이 «뒤에서» 열리고 있었다

`ProPaywall .overlay` 가 `z-index: 1200` 인데 설정 시트가 `9999` 다.
설정의 «업그레이드»를 눌러도 페이월이 시트 뒤에 그려져 **화면은 아무 일도
안 일어난 것처럼 보였다.** 결제로 가는 길 하나가 통째로 죽어 있었다.

**웹에서는 절대 안 보이는 버그다** — `iapAvailable = IAP_LIVE && 네이티브` 라
브라우저에는 그 버튼 자체가 없다. 시뮬레이터에 네이티브로 올려 직접 눌러서 찾았다.

→ `z-index: 10050` (앱에서 가장 위)

### 같이 고친 것

| 무엇 | 왜 |
|---|---|
| 광고 트는 경로 3곳 전부 Pro 가드 | 리워드만 «도달 불가»에 기대고 있었다 |
| 구독자에게 남던 **죽은 50px 띠** | `--app-anchor-ad-height` 기본값 50px 가 그대로 비어 있었다 |
| 페이월 열 때 **오퍼링 재시도** | 마운트 때 한 번만 받아서, 콜드스타트에 실패하면 세션 내내 «구매 불가» |
| 앱 번들 **472MB → 11/16MB** | `webDir` 이 마케팅 영상 455MB 가 쌓인 `out/` 을 가리켰다 |
| 웹 노출 차단 | 플래그만 켜면 브라우저에도 «못 사는 버튼»이 뜬다 (App Store 3.1.1) |

### iOS TestFlight
**1.7 (build 9)** · VALID · 내부 그룹 `Internal` · `contact@signumhq.com` 초대됨.
프리뷰 브랜치 별칭을 가리키므로 웹 수정이 재빌드 없이 반영된다.
※ 확인이 끝나면 지우고 앱스토어 정식판을 쓸 것.
...
결제 시트까지는 전부 확인했다. **애플 계정 로그인부터가 내가 넘을 수 없는 선이다**
(계정 비밀번호 입력). 남은 것은 실제로 결제가 체결됐을 때
RevenueCat 이 `pro` 엔타이틀먼트를 주는지 — 그건 대표 계정으로만 확인된다.

RevenueCat 쪽은 REST 로 직접 확인했다: iOS·Android 둘 다
`default` 오퍼링 → `$rc_monthly` → `com.signumhq.app.pro.monthly`.

### 1번을 하는 법 (대표)
1. App Store Connect → 사용자 및 액세스 → **샌드박스 테스터** 추가(실제로 쓰지 않는 이메일)
2. 아이폰 → 설정 → App Store → **샌드박스 계정**에 그 계정으로 로그인
3. `IAP_LIVE=true` 로 올린 프리뷰를 앱에서 열고 페이월 → 구매 → 「구매 복원」까지
4. 결과를 알려주면 내가 운영에 반영한다

### 한국 «유사투자자문업» — 2026-09-08 대표 지적으로 정정

내가 이 항목을 «선결 조건»으로 적어뒀는데, 대표 지적이 맞다. 과했다.

> 「단지 광고 제거 유료구독이니 상관없는것 아니야?」
> 「유료 구독을 하면 무엇인가 더 해주는것이 아니라 단지 광고제거이니까」

자본시장법상 유사투자자문업의 요건은 «**대가를 받고** 투자조언을 제공»하는 것이다.
우리 유료 티어는 **잠기는 지표가 하나도 없다** — 지표·AI 요약·리포트가 전부
무료 사용자에게도 똑같이 열려 있고, 결제로 사는 것은 «광고가 안 뜨는 화면»뿐이다.
즉 결제 대금이 조언의 대가가 아니다. ASC 구독 설명도 그렇게 적혀 있다
(`Removes every ad. Same data, same features.`).

**따라서 지금 구조에서는 유료화가 이 논점을 새로 만들지 않는다.**
(콘텐츠 자체의 성격은 유·무료와 무관한 별개 문제이고, 그건 이미 앱 전체에
금융 데이터 고지·투자조언 아님 문구로 대응하고 있다.)

**단, 이 결론을 지키는 조건이 하나 있다 — 유료가 «더 주기» 시작하면 판이 바뀐다.**
유료 전용 시그널·종목 추천·1:1 Q&A·인앱 채팅을 붙이는 순간
«대가를 받고 조언»의 모양이 된다. 그때는 변호사 판단이 필요하다.
지금 구조(광고 제거 전용, 단방향)를 유지하는 것이 방어 그 자체다.

※ 나는 변호사가 아니다. 위는 제품 구조에 대한 사실 정리이지 법률 자문이 아니다.
