...
| # | 무엇 | 누가 | 왜 |
|---|---|---|---|
| 1 | **실기기 구매·복원 테스트** | **대표** | 시뮬레이터는 실제 샌드박스 결제를 못 한다(RevenueCat 공식). 애플 계정으로 샌드박스 테스터 로그인이 필요하다 — **내가 대신 할 수 없다** |
| 2 | `IAP_LIVE = true` (`src/config/iap.ts` 한 줄) | 나 | 1번이 끝난 뒤에만. 뒤집으면 웹 배포만으로 즉시 반영된다(원격 웹셸) |
