...
**리다이렉트형은 제가 통과합니다**(Bing·Softonic 성공). 아래 둘만 팝업형(GSI)이라 막힙니다.
...
- **Softonic** — 조직 생성(SIGNUM HQ, 회사계정) + 배포약관 동의 + 앱 등록정보 **100% 작성 완료**. APK만 남음(E0)
- **홍보 이미지 공장 v2** — 합성 방식으로 재작성. v1 의 결함 3개를 실측으로 잡음:
  ①캡션이 콘텐츠를 밀어내 하단 잘림 ②스폰서 광고가 화면을 덮음 ③1080×**1919** 로 규격 1px 미달
  → v2 는 광고 제거 + 탭바 하단 실측 clip + 브랜드 캔버스 합성. 12장 재생산
- **X(@signumhq) 실측** — 105 포스트, 링크 정상 부착. **그러나 팔로워 1명**
- **Bluesky 실측** — 548 포스트, 팔로워 22명
  → 진단: 발행 기계는 정상. 문제는 «도달». 팔로워 0 계정에 노출을 주는 경로는
    검색엔진·앱스토어 검색·디렉터리 등재뿐 → 오늘 작업 방향이 맞다
