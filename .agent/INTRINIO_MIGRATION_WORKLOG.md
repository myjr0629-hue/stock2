...
## 2026-08-29 (토) 오후 — 앱 실화면 검증에서 «지어낸 값» 발견

### ★★ 가장 심각한 발견 — 화면이 지표를 «합성»하고 있었다
앱 Intel 실화면에서 섹터마다 `D.POOL 69% / 53% / 57%` 가 보였다.
다크풀은 현재 플랜에 틱이 없어 **측정 자체가 불가**한데도.

공급원을 하나씩 막았는데 계속 나왔다 — **4갈래였다.**
| # | 공급원 | 조치 |
|---|---|---|
| 1 | `cache:inst-last:*` (Massive 시절 잔재) | 읽는 입구에 `tickDataAvailable()` 게이트 |
| 2 | `analysis` Redis 캐시 (MSFT 94.6% 같은 깨진 값) | 게이트 뒤로 |
| 3 | DynamoDB `institutional` | 게이트 뒤로 |
| 4 | **화면에서 직접 합성** | ← 진짜 원인 |

4번이 본체였다. `intel/page.tsx` 에서:
```ts
...
```
**감마 펄스에서 다크풀 비중을 만들어내고 있었다.** 새로고침마다 값이 바뀌었다.
이건 «0 을 사실로 주장»하는 것보다 나쁘다 — 없는 측정을 구체적 숫자로 만든 것이다.

### 합성값을 걷어내니 진짜 결함이 드러났다
제거 후 `WHALE —` 이 되었다. 확인해 보니 데이터가 없는 게 아니라
`intel/fast` 응답에 `whaleIndex` 필드가 **빠져 있었을 뿐**이었다
(`watchlist/batch` 는 whale 70/70/68 을 정상 반환).
→ 합성값이 파이프라인 공백을 가리고 있었다. **이게 합성값의 진짜 해악이다.**
→ `intel/fast` 에 실제 계산값 추가.

### 최종 화면 (실측)
```
수정 전:  D.POOL 64%  WHALE 64  SQUEEZE 53%   ← 앞 둘이 합성
수정 후:  D.POOL —    WHALE 64  SQUEEZE 53%   ← 측정 불가만 «—»
          GEX -825.99M · PCR 0.62 · NET PREM +$832.0M  (실측값 유지)
```

### Flow 화면에서도 같은 위반
```
수정 전:  다크풀 블록 $0 · 비중 0.0% · 블록 강도 0 · 순방향 +$0
수정 후:  다크풀 블록 —  · 비중 —    · 블록 강도 —  · 순방향 —
```
+ **압력 점수 가중치 재정규화**: 다크풀(0.3)을 «중립 0 표»로 채우지 않고
  남은 신호(고래)에 재분배. 없는 신호가 결론을 중립으로 끄는 것도 왜곡이다.
  (확신도 59%→60%, 압력 +40→+42 로 변화)

### 앱 화면 전수 검증 결과 (2026-08-29 토 · 휴장)
| 화면 | 상태 |
|---|---|
| Guardian | RLSI 49 · VIX 14.4 · CLOSED 정확 · **Breadth «본장 전용/—»** (설계대로) |
| Command | 정규장 $217.58(-4.56%) / **POST $217.86(+0.13%) 분리 정확** · NBBO 스프레드 0.139% |
| Flow | 옵션 지표 정상 · 다크풀 «—» · `Source: ESTIMATED` 라벨 정직 |
| Intel | 10섹터 정상 · D.POOL «—» · WHALE/SQUEEZE 실측 |

### 검증 도구 현황
```
엔드포인트 감사   : 통과 50 · 주의 2 · 실패 0
Massive 커버리지  : 라우팅 25 · 의도적 미지원 7 · 누수 0
```
주의 2건은 둘 다 정상 — 휴장 중 breadth 중립, NVDA 마지막 배당일(6/04).
