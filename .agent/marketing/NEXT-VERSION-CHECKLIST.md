...
⚠️ **이 표의 한계 — «수요»는 못 쟀다.** 보증하는 것은 «경쟁 없음»까지다.
  2026-09-20 에 수요 신호를 두 갈래로 시도했고 둘 다 막혔다: 애플 자동완성(MZSearchHints)은 어떤 질의든 315바이트 빈 plist,
  Google Search Console 은 자격증명이 없다. **그래서 교체안은 «못 이기는 토큰 → 비어 있는 토큰» 교환으로만 제한했다** —
  검색량을 모르는 말을 «새로» 밀어 넣지 않았다. GSC 가 열리면 실제 사용자 질의로 이 표를 한 번 더 거른다.
