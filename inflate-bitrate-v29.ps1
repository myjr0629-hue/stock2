...
$InputVideo = "out/market_pressure_brief_v29_premium_intelligence_cut.mp4"
$OutputVideo = "out/market_pressure_brief_v29_premium_intelligence_cut_cbr.mp4"
...
Write-Host "--- INFLATING V29 BITRATE TO 15MBPS CBR ---"
& $FFmpegPath -y -i $InputVideo -b:v 15M -minrate 12M -maxrate 18M -bufsize 30M -c:v libx264 -x264-params nal-hrd=cbr -c:a aac -b:a 320k $OutputVideo
...
$silenceRaw | Out-File -FilePath "out/review/v29_silencedetect.txt" -Encoding utf8
...
$probeOutput | Out-File -FilePath "out/review/v29_ffprobe.json" -Encoding utf8
$probeOutput

Write-Host "`n--- RUNNING STORYBOARD CONTACT SHEET GENERATOR ---"
node make-contact-sheet-v29.js
