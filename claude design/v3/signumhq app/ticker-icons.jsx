/* global React */
/* ============================================================
   SIGNUM HQ — TickerIcon
   Rule 1: every major ticker gets a hi-fi brand symbol badge.
   Custom monogram badges in each brand's hue (original art —
   not trademarked logo reproductions).
   ============================================================ */
...
function TickerIcon({ sym, size = 18 }) {
  const r = TICKER_REGISTRY[sym] || { c1: '#1e293b', c2: '#475569', g: (sym || '?').slice(0, 2) };
  const id = 'tk-' + sym + '-' + size;
  const fs = r.g.length >= 3 ? size * 0.30 : r.g.length === 2 ? size * 0.40 : size * 0.50;
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ flexShrink: 0, display: 'block' }} aria-label={sym}>
      <defs>
      ...
      <text x="50%" y="52%" dominantBaseline="central" textAnchor="middle"
        fill="#fff" fontFamily="Inter, sans-serif" fontWeight="800" fontSize={fs}
        style={{ letterSpacing: '-0.02em' }}>{r.g}</text>
    </svg>
  );
}

Object.assign(window, { TickerIcon, TICKER_REGISTRY });
