...
const videoPath = 'out/market_pressure_brief_v27_upload_master.mp4';
const outputPath = 'out/review/v27_contact_sheet.jpg';
...
try {
  ...
  // We specify the select filter with double-quotes and escaped comma for select parameters
...
let html = `<html>
<head>
  <title>V27 Institutional Upload Master - Contact Sheet</title>
  <style>
...
<body>
  <h1>MARKET PRESSURE BRIEF V27</h1>
  <p>Collision-Free Institutional Upload Master - Visual Storyboard Review</p>
  <div class="grid">
`;
...
frames.forEach((f, idx) => {
  ...
  const stillName = `v27_frame_${String(frameIdx).padStart(3, '0')}.jpg`;
...
html += `
  ...
  <div style="margin-top: 50px; font-size: 14px; color: #64748b;">
    Generated programmatically via SignumHQ Shorts Engine - Mission 34 V27
  </div>
</body>
</html>`;

fs.writeFileSync(path.join(outDir, 'v27_contact_sheet.html'), html);
console.log('Successfully created out/review/v27_contact_sheet.html and extracted all storyboard stills!');
